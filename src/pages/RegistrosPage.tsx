...
export function RegistrosPage({ onVerCliente }: Props) {
  ...
  const diasAutoRechazo =
    useAjustes(null).data?.sistema.diasAutoRechazo ??
    AJUSTES_DEFAULT.sistema.diasAutoRechazo

  const [searchParams, setSearchParams] = useSearchParams()
  const [busqueda, setBusqueda] = React.useState('')

  const ESTADOS_VALIDOS: EstadoPresupuesto[] = [
    'borrador',
    'pendiente',
    'aceptado',
    'rechazado',
  ...
  const tab: Tab =
    searchParams.get('tab') === 'pagos' ? 'pagos' : 'presupuestos'
  ...
  const presupuestosFiltrados = React.useMemo(() => {
    ...
    return obras
      ...
      .map((o) => ({
        ...
        totales: calcularTotalesObra(
          o,
          pagos.filter((p) => p.obraId === o.id),
        ),
  ...
  const ESTADOS: (EstadoPresupuesto | 'todos')[] = [
    'todos',
    'borrador',
    'pendiente',
    'aceptado',
    ...
    >
      ...
      {tab === 'presupuestos' ? (
        <div className="grid grid-cols-3 overflow-hidden rounded-xl border border-border/60 bg-card/45">
          <ResumenDato
            label="Pendientes"
            value={String(resumenPresupuestos.pendientes)}
            icon={<Clock3 />}
            tone="warning"
          />
          <ResumenDato
            label="Aceptados"
            value={String(resumenPresupuestos.aceptados)}
            icon={<FileText />}
          />
          <ResumenDato
            label="Valor ventas"
            value={`$${formatMoney(resumenPresupuestos.valorAceptado)}`}
        ...
        <div className="grid grid-cols-3 overflow-hidden rounded-xl border border-border/60 bg-card/45">
          <ResumenDato
            label="Cobrado"
            value={`$${formatMoney(resumenPagos.cobrados)}`}
            icon={<CircleDollarSign />}
            tone="success"
          />
          <ResumenDato
            label="Recibos"
            value={String(resumenPagos.vigentes)}
            icon={<Receipt />}
          />
          <ResumenDato
            label="Anulados"
            value={String(resumenPagos.anulados)}
            icon={<Ban />}
            tone="danger"
          />
        </div>
            ...
            {ESTADOS.map((e) => (
              ...
              >
                {e === 'todos' ? (
                  ...
                  <EstadoPresupuestoBadge estado={e} size="sm" />
                )}
              </button>
            ))}
          </div>
                ...
                >
                  <div className="overflow-hidden rounded-xl border border-border/60 bg-card/60 backdrop-blur-sm transition-all hover:border-primary/40 hover:bg-card hover:shadow-md dark:bg-card/50 dark:hover:bg-card/80">
                    ...
                    {(obra.estadoPresupuesto === 'aceptado' ||
                      obra.estadoPresupuesto === 'pendiente') && (
...
function ResumenDato({
  ...
  return (
    <div className="min-w-0 border-l border-border/50 px-2 py-3 first:border-l-0 sm:px-4">
      ...
      <p
        className="money truncate text-sm font-bold sm:text-base"
        title={value}
      >
        {value}
      </p>
